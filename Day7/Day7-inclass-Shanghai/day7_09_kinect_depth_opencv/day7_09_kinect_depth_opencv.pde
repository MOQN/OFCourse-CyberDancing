...
void draw() {
  ...
  //opencv.erode();  // the pixel area shrinks - to remove 1px noises
  //opencv.dilate(); // the pixel area expands - to fill the holes
  
  opencv.erode();
  opencv.erode();
  
  PVector pos = opencv.max(); // return the brigtest position (x, y, (z))
  kinectX = lerp(kinectX, pos.x, 0.1);
  kinectY = lerp(kinectY, pos.y, 0.1);
